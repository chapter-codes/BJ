...
const HeroSection = ({loadState}:HeroSectionProps)=>{
    ...
    return <div id='hero' className="relative flex items-center justify-center text-white w-full h-[400px] sm:h-[600px] lg:h-[calc(100svh-var(--header-height))] max-h-[1000px] bg-cover bg-center bg-no-repeat ">
        <div className="absolute inset-0 w-full h-full overflow-hidden">
            <img src={hero} 
                alt="" 
                className={`absolute inset-0 w-full h-full object-cover transition-transform duration-1000 ${isLoaded?'scale-100 opacity-100':'scale-[3] opacity-30'}`}
        ...
        <CustomDiv className='relative flex flex-col justify-center items-center gap-2 mb-0! overflow-hidden'>
            <h1 
                className={`text-2xl lg:text-[2.5rem] text-center lg:text-center font-semibold max-w-[300px] md:max-w-[400px] lg:max-w-[700px] transition-all duration-[1000ms] delay-500 relative ${isLoaded?'translate-y-0': '-translate-y-full'}`}>
